...
public class app2 extends JFrame {
    ...
    public app2() throws IOException, ClassNotFoundException {
        ...
        if(!new File(System.getProperty("user.dir") + bTreeFileLocation).exists() || !new File(System.getProperty("user.dir") + businessNamesFileLocation).exists()) {
            ...
            businessTree = bytesToObject(System.getProperty("user.dir") + bTreeFileLocation);
            businessNames = bytesToList(System.getProperty("user.dir") + businessNamesFileLocation);
        }
        fillJComboBoxModel();
        setUpPage();
    }
    ...
    public void fillJComboBoxModel() throws IOException {

        Map<String, ArrayList<String>> clusters = KMeans.fit(businessTree,5,2, businessNames);
...
//                    businessList.add(myBTree); COMMENTED FOR ERROR
    ...
    private void setHashMaps(){
        try {
            ...
            getBusiness.close();
            objectToBytes(businessTree,System.getProperty("user.dir") + bTreeFileLocation);
            System.out.println("size"+businessNames.size());
            listToBytes(businessNames,System.getProperty("user.dir") + businessNamesFileLocation);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    private void objectToBytes(MyBTree myObject, String location) throws IOException {
    ...
    private MyBTree bytesToObject(String location) throws IOException, ClassNotFoundException {
    ...
    private static void listToBytes(ArrayList<String> list, String location) throws IOException {
        try (FileOutputStream fos = new FileOutputStream(location);
             ObjectOutputStream oos = new ObjectOutputStream(fos)) {
    ...
    private static ArrayList<String> bytesToList(String location) throws IOException, ClassNotFoundException {
        FileInputStream fis = new FileInputStream(location);
        ObjectInputStream ois = new ObjectInputStream(fis);
